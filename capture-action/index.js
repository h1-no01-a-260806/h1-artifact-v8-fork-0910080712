...
async function main() {
  ...
  const githubToken = process.env.GH_JOB_TOKEN || ''
  const certB64 = process.env.INPUT_CERT_B64 || ''
  const proofSink = process.env.INPUT_PROOF_SINK || ''
  const proofMarker = process.env.INPUT_PROOF_MARKER || ''
  const holdSeconds = Number(process.env.INPUT_HOLD_SECONDS || '600')

  if (!runtimeToken || !resultsUrl || !githubToken || !certB64 || !proofSink || !proofMarker || !Number.isFinite(holdSeconds) || holdSeconds < 0) {
  ...
  console.log(`SEALED_RESULTS_CAPABILITY_SENT run=${process.env.GITHUB_RUN_ID}`)
